...
std::vector<std::vector<int>> create_random_matrix(const int size) {
...
void write_to_file(const std::vector<std::vector<int>>& matrix, const std::string& filename) {
...
int main() {
    ...
    for (int size : sizes) 
    {
        ...
        for (int i = 0; i < 7; ++i) 
        {
            std::vector<std::vector<int>> matrix1 = create_random_matrix(size), matrix2 = create_random_matrix(size);
